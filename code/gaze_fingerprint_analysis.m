function gaze_fingerprint_analysis(WEIGHT,PLOT,nperm,RUNONSERVER,NWORKERS)
...
%   Example usage:
%
%   WEIGHT = 'duration'; PLOT = false; nperm = 0; RUNONSERVER = false; NWORKERS = 5;
%   gaze_fingerprint_classifier_opt2(WEIGHT, PLOT, nperm, RUNONSERVER, NWORKERS);
%

%% save input arguments to a structure
input_args = parse_input_args(WEIGHT, PLOT, nperm, RUNONSERVER, NWORKERS);

%% get experiment information
exp_info = get_exp_info(WEIGHT);
exp_info.nstim = 700; % hack to make it run faster for code testing purposes

%% path information
path_info = get_paths(RUNONSERVER);

%% get pheno data
pheno_fname = fullfile(path_info.datapath,'pheno','pheno_OSIE_fingerprint_analysis.csv');
...
function input_args = parse_input_args(WEIGHT, PLOT, nperm, RUNONSERVER, NWORKERS)

input_args.WEIGHT = WEIGHT;
input_args.PLOT = PLOT;
input_args.nperm = nperm;
...
path_info.resultpath = fullfile(path_info.rootpath,'results');
...
function exp_info = get_exp_info(WEIGHT)

% set file stem
exp_info.fstem = sprintf('weight_%s',WEIGHT);
...
exp_info.nstim = 700; % number of stimuli %change to 1 (should be 700) for test purposes
...
for isub = 1:length(target.subids)
    ...
    if ismember(sub_pheno_data.data_directory_name(1),'external_datasets')
        fpath2use = fullfile(fpath2use,sub_pheno_data.dataset{1});
    end % if ismember(sub_pheno_data.data_directory_name(1),'external_datasets')

    % target session 1
    fpath2use = fullfile(fpath2use,sub_pheno_data.subj_ID{1}, ...
        sub_pheno_data.target_session_name{1},'fixations');
    fname2use = dir(fullfile(fpath2use,'*fixations.mat'));
    fname2use = fullfile(fpath2use,fname2use.name);
    disp(fname2use);

    tmp = load(fname2use);
    if ismember(sub_pheno_data.dataset(1),'IIT')
        fix_data.sess01{isub} = tmp.data.fixations;
    elseif ismember(sub_pheno_data.dataset(1),'Gi')
        fix_data.sess01{isub} = tmp.fixations;
    end % if ismember(sub_pheno_data.dataset(1),'IIT')
    ...
    % target session 2
    fpath2use = fullfile(fpath2use,sub_pheno_data.subj_ID{1}, ...
        sub_pheno_data.comparison_session_name{1},'fixations');
    fname2use = dir(fullfile(fpath2use,'*fixations.mat'));
    fname2use = fullfile(fpath2use,fname2use.name);
    disp(fname2use);
    ...
    elseif ismember(sub_pheno_data.dataset(1),'Gi')
        fix_data.sess02{isub} = tmp.fixations;
    end % if ismember(sub_pheno_data.dataset(1),'IIT')
...
if input_args.nperm>0
    ...
    % pre-allocate perm arrays
    perm_accuracy = zeros(nstim, input_args.nperm);
    perm_entropy_per_stim_sess1 = zeros(nsubs, nstim, input_args.nperm);
    perm_entropy_per_stim_sess2 = zeros(nsubs, nstim, input_args.nperm);
    perm_fingerprinting_ratios = zeros(nstim, input_args.nperm);
    perm_mean_intrasubject_r = zeros(nstim, input_args.nperm);
    perm_mean_mean_intersubject_r = zeros(nstim, input_args.nperm);
    perm_fingerprintable_stimuli_per_sub = zeros(nsubs, input_args.nperm);

    parfor iperm = 1:input_args.nperm

        % pre-allocate perm arrays ----------------------------------------
        entropy_per_stim_perm = zeros(nsubs, 2, nstim);
        id_mat_perm = nan(nsubs, nsubs, nstim);
        fingerprint_mat_perm = nan(nsubs, nsubs, nstim);
        sub_BY_stim_IDarray_perm = zeros(nsubs, nstim);
        intrasubject_r_perm = zeros(nsubs, nstim);
        mean_intersubject_r_perm = zeros(nsubs, nstim);
        fingerprint_ratios_perm = zeros(nsubs, nstim);
        nfp_subs_perm = zeros(nstim,1);
        acc_perm = zeros(nstim,1);
        mean_fp_ratio_perm = zeros(nstim,1);

        % permute session 2 subject ordering ------------------------------

        % set seed for reproducibility
        rng(iperm);

        % generate permuted ordering
        permuted_ordering = randperm(nsubs);

        % reorder session 2 data
        perm_target = target;
        perm_target.fixations.sess02 = perm_target.fixations.sess02(permuted_ordering);

        for istim = 1:nstim

            disp(fprintf('Permutation %d, Stimulus %03d', iperm, istim));

            % compute target subs heatmaps --------------------------------
            hmap_res_perm = get_heatmaps(perm_target, istim, exp_info, input_args, path_info);

            % calculate intrasubject and intersubject similarity ----------
            gsa_res_tmp_perm = compute_gaze_similarity(hmap_res_perm, perm_target);

            % do kNN classifier and fingerprint ratio ---------------------
            fp_res_perm = fingerprint_classification(gsa_res_tmp_perm);

            % put results together in these arrays ------------------------
            id_mat_perm(:,:,istim) = gsa_res_tmp_perm.id_mat;
            fingerprint_mat_perm(:,:,istim) = fp_res_perm.fingerprint_mat;

            % grab correct or incorrect classification for each target subject
            sub_BY_stim_IDarray_perm(:,istim) = fp_res_perm.fingerprint_mat(logical(eye(size(fp_res_perm.fingerprint_mat))));

            % grab entropy data
            entropy_per_stim_perm(:,:,istim) = gsa_res_tmp_perm.entropy;

            % grab intrasubject correlations
            intrasubject_r_perm(:,istim) = gsa_res_tmp_perm.intrasubject_r;

            % grab mean intersubject correlations
            mean_intersubject_r_perm(:,istim) = gsa_res_tmp_perm.mean_intersubject_r;

            % grab fingerprint ratios
            fingerprint_ratios_perm(:,istim) = fp_res_perm.fingerprint_ratios;

            % find number of subjects that can be fingerprinted
            nfp_subs_perm(istim,1) = sum(sub_BY_stim_IDarray_perm(:,istim), 'omitnan');

            % compute percent accuracy for fingerprinting using maximal correlation
            acc_perm(istim,1) = nfp_subs_perm(istim,1)./length(target.subids);

            % compute mean fingerprint_ratio across subjects
            mean_fp_ratio_perm(istim,1) = mean(fp_res_perm.fingerprint_ratios, 'omitnan');

        end % for istim

        % perm entropy per stim
        perm_entropy_per_stim_sess1(:,:,iperm) = squeeze(entropy_per_stim_perm(:,1,:));
        perm_entropy_per_stim_sess2(:,:,iperm) = squeeze(entropy_per_stim_perm(:,2,:));

        % compute mean intrasubject and mean mean intersubject correlations
        perm_mean_intrasubject_r(:,iperm) = mean(intrasubject_r_perm,1, 'omitnan');
        perm_mean_mean_intersubject_r(:,iperm) = mean(mean_intersubject_r_perm,1, 'omitnan');

        % permutation accuracy for each stimulus
        perm_accuracy(:,iperm) = acc_perm;

        % permutation mean fingerprint ratio for each stimulus
        perm_fingerprinting_ratios(:,iperm) = mean_fp_ratio_perm;

        % permutation number of fingerprintable stimuli
        perm_fingerprintable_stimuli_per_sub(:,iperm) = reshape(sum(sub_BY_stim_IDarray_perm,2),nsubs,1);

    end % parfor iperm

    % put results into gsa_res.perm ---------------------------------------
    gsa_res.perm.nperm = input_args.nperm;

    % permutation accuracy for each stimulus
    gsa_res.perm.accuracy = perm_accuracy;

    % permutation mean fingerprint ratio for each stimulus
    gsa_res.perm.fingerprinting_ratios = perm_fingerprinting_ratios;

    % permutation mean intrasubject correlation for each stimulus
    gsa_res.perm.mean_intrasubject_r = perm_mean_intrasubject_r;

    % permutation mean intersubject correlation for each stimulus
    gsa_res.perm.avg_mean_intersubject_r = perm_mean_mean_intersubject_r;

    % permutation number of fingerprintable stimuli
    gsa_res.perm.fingerprintable_stimuli_per_sub = perm_fingerprintable_stimuli_per_sub;

    % permutation entropy per stim
    gsa_res.perm.entropy_per_stim_sess1 = perm_entropy_per_stim_sess1;
    gsa_res.perm.entropy_per_stim_sess2 = perm_entropy_per_stim_sess2;
...
mask = logical(eye(size(results.id_mat)));
...
% save out permutation results --------------------------------------------
if gsa_res.perm.nperm>0

    % permutation names
    perm_names = cell(1,gsa_res.perm.nperm);
    for iperm = 1:gsa_res.perm.nperm
        perm_names{iperm} = sprintf('perm%06d',iperm);
    end % for iperm

    % ---------------------------------------------------------------------
    % write out results for number of fingerprintable stimuli under permutations
    tab2write = cell2table([gsa_res.subids, num2cell(gsa_res.perm.fingerprintable_stimuli_per_sub)],'VariableNames',[{'subids'},perm_names]);
    fname2save = fullfile(path_info.resultpath, sprintf('%s_classification_nfingerprintableStimuli_perSub_perm.csv',exp_info.fstem));
    writetable(tab2write,fname2save);

    % ---------------------------------------------------------------------
    % write out results for permutation accuracy per each stimulus
    tab2write = cell2table([stim_names_str',num2cell(gsa_res.perm.accuracy)],'VariableNames',[{'stimulus'},perm_names]);
    fname2save = fullfile(path_info.resultpath, sprintf('%s_classification_accuracy_perStim_perm.csv',exp_info.fstem));
    writetable(tab2write,fname2save);

    % ---------------------------------------------------------------------
    % mean intrasubject correlations perm ---------------------------------
    tab2write = cell2table([stim_names_str',num2cell(gsa_res.perm.mean_intrasubject_r)],'VariableNames',[{'stimulus'},perm_names]);
    fname2save = fullfile(path_info.resultpath, sprintf('%s_mean_intrasubjectR_perStim_perm.csv',exp_info.fstem));
    writetable(tab2write,fname2save);

    % ---------------------------------------------------------------------
    % mean intersubject correlations perm ---------------------------------
    tab2write = cell2table([stim_names_str',num2cell(gsa_res.perm.avg_mean_intersubject_r)],'VariableNames',[{'stimulus'},perm_names]);
    fname2save = fullfile(path_info.resultpath, sprintf('%s_mean_mean_intersubjectR_perStim_perm.csv',exp_info.fstem));
    writetable(tab2write,fname2save);

    % ---------------------------------------------------------------------
    % fingerprint ratios perm ---------------------------------------------
    tab2write = cell2table([stim_names_str',num2cell(gsa_res.perm.fingerprinting_ratios)],'VariableNames',[{'stimulus'},perm_names]);
    fname2save = fullfile(path_info.resultpath, sprintf('%s_mean_fingerprintratios_perStim_perm.csv',exp_info.fstem));
    writetable(tab2write,fname2save);
