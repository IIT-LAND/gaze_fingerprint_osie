...
data2plot$feature = factor(data2plot$feature, levels = rev(c("watchability","touched","human","social","face","taste","touch","motion","emotion","text","operability","gazed","nonsocial","smell","sound","animal")))
...
a = data.frame(res$Best.nc[1,]); colnames(a) = "k"; a$index = rownames(a)
best_nk = as.numeric(names(table(a$k))[table(a$k)==max(table(a$k))])
res = NbClust(data = t(enrich_res_OR), method = clust_method, index="kl")
...
row_cols[row_cols==tmp_row_cols[1]] = new_row_cols[4]
row_cols[row_cols==tmp_row_cols[2]] = new_row_cols[3]
