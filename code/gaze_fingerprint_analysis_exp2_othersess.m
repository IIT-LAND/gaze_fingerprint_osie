function gaze_fingerprint_analysis_exp2_othersess(WEIGHT,PLOT,RUNONSERVER,NWORKERS,other_session,test)
...
%   other_session = "session_03"; test = true;
...
target.fixations = get_fix_data(target, pheno_data, path_info, other_session);
...
path_info.resultpath = fullfile(path_info.rootpath,sprintf('results_exp2_%s', input_args.other_session));
...
function fix_data = get_fix_data(target, pheno_data, path_info, other_session)
...
fix_data.other_session{1, nsubs} = [];
...
for isub = 1:length(target.subids)
    ...
    fpath2use_session_other = fullfile(fpath2use,sub_pheno_data.subj_ID{1}, ...
                                other_session,'fixations');
    fdir2use_session_other = dir(fullfile(fpath2use_session_other,'*fixations.mat'));

    for file = fdir2use_session_other'

        fname2use = fullfile(fpath2use_session_other,file.name);
        disp(fname2use);

        if ismember(sub_pheno_data.comparison_use(1),'Yes')
            tmp = load(fname2use);
            fix_data.other_session{isub} = tmp.data.fixations;
        else
            disp("no fixation data loaded") 
        end % contains(fname2use, sub_pheno_data.subj_ID{1})

    end % file = fdir2use_session_other'
...
%for istim = 1:nstim
    ...
    % compute session 1 to other session intras
    other_session_results = compute_other_sess_corrs(hmap_res)
    ...
    fp_res = fingerprint_classification(gsa_res_tmp, other_session_results);
    ...
    % grab intrasubject correlations for session 1 to other
    intrasubject_r(:,istim) = other_session_results.intrasubject_r;
...
fixhmap_other_tmp = zeros(nsubs,stim_dim(1)*stim_dim(2));
...
for isub = 1:nsubs
    ...
    if ~isempty(target.fixations.sess02{isub})
        ...
        fixhmap_other_tmp(isub,:) = nan(1,size(fixhmap_other_tmp,2));
        empty_hmap(isub,3) = 1;
    end

    % other session
    % fix so it only cares about people with session 3 fixation data

    if ~isempty(target.fixations.other_session{isub})
        fixationdata2use = target.fixations.other_session{isub}{istim};
        mask = isempty(fixationdata2use);
        if ~mask
            res = fixationHeatmap(fixationdata2use, imgname, smoothing_radius, 0, NaN, WEIGHT);
            fixhmap_other_tmp(isub,:) = res(:);
        else
            fixhmap_other_tmp(isub,:) = nan(1,size(fixhmap_other_tmp,2));
            empty_hmap(isub,3) = 1;
        end % if ~mask
...
results.fix_hmap_other = fixhmap_other_tmp;
results.empty_hmap = empty_hmap;
results.entropy_nrow = entropy_nrow;
results.entropy_ncol = entropy_ncol;

end % function get_heatmaps
%% compute other intra corrs only
function other_session_results = compute_other_sess_corrs(hmap_res)

other_session_results.istim = hmap_res.istim;
other_session_results.nsubs = hmap_res.nsubs;
other_session_results.subids = hmap_res.subids;

other_session_results.id_mat = nan(other_session_results.nsubs,other_session_results.nsubs);
other_session_results.intrasubject_r = nan(other_session_results.nsubs,1);

% calculate intrasubject correlation ----------------------------------
disp('Computing intrasubject correlations');
other_session_results.id_mat = corr(hmap_res.fix_hmap1',hmap_res.fix_hmap_other');
mask = logical(eye(size(other_session_results.id_mat))); % extract the diagonal containg sub_i by sub_i values
other_session_results.intrasubject_r = other_session_results.id_mat(mask);

end % function compute intra_sub corr only
...
function fp_res = fingerprint_classification(gsa_res_tmp, other_session_results)
...
% grab other session intra-corr-values 
diag_other = other_session_results.intrasubject_r(:,1);

% grab session 1 to session 2 id mat
id_mat = gsa_res_tmp.id_mat;

% replace session 1 to session 2 intras, with session 1 to session 'other'
% intras
id_mat(1:1+size(id_mat,1):end) = diag_other;
